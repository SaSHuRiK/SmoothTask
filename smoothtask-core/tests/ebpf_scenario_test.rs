...
fn test_ebpf_high_load_scenario() {
    ...
    println!("Время выполнения 100 сборов метрик с агрессивным кэшированием: {:?}", duration);
...
fn test_ebpf_low_latency_scenario() {
    ...
    println!("Метрики без кэширования: {:.1}% vs {:.1}% CPU", metrics1.cpu_usage, metrics2.cpu_usage);
}
...
fn test_ebpf_detailed_monitoring_scenario() {
    ...
    if let Some(syscall_details) = metrics.syscall_details {
        ...
        println!("Детализированная статистика системных вызовов: {} записей", syscall_details.len());
    }
    
    if let Some(network_details) = metrics.network_details {
        assert!(!network_details.is_empty());
        println!("Детализированная статистика сети: {} записей", network_details.len());
    }
    
    if let Some(filesystem_details) = metrics.filesystem_details {
        assert!(!filesystem_details.is_empty());
        println!("Детализированная статистика файловой системы: {} записей", filesystem_details.len());
    }
}
...
fn test_ebpf_performance_comparison_scenario() {
    ...
    println!("Производительность без кэширования: {:?}", no_cache_duration);
    println!("Производительность с кэшированием: {:?}", with_cache_duration);
