...
async fn test_vm_auto_scaling_complete_cycle() -> Result<()> {
    ...
    let metrics = monitor_vm_load("test_vm")?;
...
async fn test_vm_scaling_overloaded_scenario() -> Result<()> {
    ...
    let mut metrics = monitor_vm_load("vm2")?; // vm2 имеет высокую нагрузку
...
async fn test_vm_scaling_underutilized_scenario() -> Result<()> {
    ...
    let mut metrics = monitor_vm_load("vm3")?; // vm3 имеет низкую нагрузку
...
async fn test_vm_scaling_resource_calculation() -> Result<()> {
    ...
    assert!(
        scaling_plan.network_scaling_factor >= 0.5 && scaling_plan.network_scaling_factor <= 2.0
    );
...
async fn test_vm_scaling_multiple_vms() -> Result<()> {
    ...
    for vm_id in vm_ids.iter() {
        // Мониторинг нагрузки
        let metrics = monitor_vm_load(vm_id)?;
        ...
        let result = apply_vm_auto_scaling(vm_id, &scaling_plan, &metrics)?;
...
async fn test_vm_scaling_error_handling() -> Result<()> {
    ...
    let result = apply_vm_auto_scaling("nonexistent_vm", &scaling_plan, &metrics)?;
    assert!(!result.success);
...
async fn test_vm_scaling_statistics() -> Result<()> {
    ...
    let sum = analysis.cpu_utilization
        + analysis.memory_utilization
        + analysis.disk_utilization
