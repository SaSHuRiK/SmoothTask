...
fn test_log_analysis_with_filtering() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        metrics_logger.log_metrics("ERROR: Test error 1").await.expect("log error 1");
        metrics_logger.log_metrics("ERROR: Test error 2").await.expect("log error 2");
        metrics_logger.log_metrics("WARN: Test warning").await.expect("log warning");
        metrics_logger.log_metrics("INFO: Test info").await.expect("log info");
        metrics_logger.log_metrics("DEBUG: Test debug").await.expect("log debug");
        ...
        let warn_analysis = integration.enhanced_log_analysis("metrics", Some("warning"), None, None).await.expect("warn analysis");
        assert!(warn_analysis.filtered_entries >= 1);
        assert!(warn_analysis.warning_count >= 1);
        
        // Тестируем фильтрацию по уровню INFO
        let info_analysis = integration.enhanced_log_analysis("metrics", Some("info"), None, None).await.expect("info analysis");
...
fn test_log_analysis_with_pattern_search() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        metrics_logger.log_metrics("ERROR: System failure detected").await.expect("log 1");
        metrics_logger.log_metrics("WARN: System approaching limits").await.expect("log 2");
        metrics_logger.log_metrics("INFO: System operating normally").await.expect("log 3");
        metrics_logger.log_metrics("DEBUG: System diagnostics complete").await.expect("log 4");
        
        // Тестируем поиск по паттерну "System"
        let system_analysis = integration.enhanced_log_analysis("metrics", None, Some("System"), None).await.expect("system analysis");
        assert!(system_analysis.matching_entries >= 4);
        
        // Тестируем поиск по паттерну "failure"
        let failure_analysis = integration.enhanced_log_analysis("metrics", None, Some("failure"), None).await.expect("failure analysis");
        assert!(failure_analysis.matching_entries >= 1);
        
        // Тестируем поиск по паттерну "complete"
        let complete_analysis = integration.enhanced_log_analysis("metrics", None, Some("complete"), None).await.expect("complete analysis");
...
fn test_log_search_functionality() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        metrics_logger.log_metrics("Non-test message").await.expect("log 4");
        ...
        let empty_results = integration.log_search("metrics", "nonexistent", false, 10).await.expect("empty search");
...
fn test_log_filtering_functionality() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        metrics_logger.log_metrics("ERROR: Another error").await.expect("log error 2");
        ...
        let combined_results = integration.log_filtering("metrics", &combined_criteria, 10).await.expect("combined filter");
...
fn test_log_analysis_edge_cases() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let empty_analysis = integration.enhanced_log_analysis("metrics", None, None, None).await.expect("empty analysis");
        assert_eq!(empty_analysis.total_entries, 0);
        assert_eq!(empty_analysis.filtered_entries, 0);
        assert!(empty_analysis.analysis_summary.contains("Log file does not exist"));
...
fn test_log_analysis_multiple_log_types() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        metrics_logger.log_metrics("Metrics log entry").await.expect("log metrics");
        classify_logger.log_classify("Classify log entry").await.expect("log classify");
        policy_logger.log_policy("Policy log entry").await.expect("log policy");
        
        // Анализируем разные типы логов
        let metrics_analysis = integration.enhanced_log_analysis("metrics", None, None, None).await.expect("metrics analysis");
        assert!(metrics_analysis.total_entries >= 1);
        assert_eq!(metrics_analysis.log_type, "metrics");
        
        let classify_analysis = integration.enhanced_log_analysis("classify", None, None, None).await.expect("classify analysis");
        assert!(classify_analysis.total_entries >= 1);
        assert_eq!(classify_analysis.log_type, "classify");
        
        let policy_analysis = integration.enhanced_log_analysis("policy", None, None, None).await.expect("policy analysis");
