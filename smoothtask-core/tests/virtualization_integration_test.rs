...
async fn test_virtualization_multiple_vms() {
    ...
    for (i, vm_id) in vm_ids.iter().enumerate() {
        ...
        assert_eq!(metrics.cpu_usage, expected.0, "CPU usage mismatch for VM {}", vm_id);
        assert_eq!(metrics.memory_usage, expected.1, "Memory usage mismatch for VM {}", vm_id);
        assert_eq!(metrics.disk_read_bytes, expected.2, "Disk read bytes mismatch for VM {}", vm_id);
        assert_eq!(metrics.disk_write_bytes, expected.3, "Disk write bytes mismatch for VM {}", vm_id);
        assert_eq!(metrics.disk_read_ops, expected.4, "Disk read ops mismatch for VM {}", vm_id);
        assert_eq!(metrics.disk_write_ops, expected.5, "Disk write ops mismatch for VM {}", vm_id);
    }
}
...
async fn test_virtualization_error_handling() {
    ...
    for vm_id in unknown_vms.iter() {
        let result = collect_vm_metrics(vm_id);
        assert!(result.is_ok(), "Should handle unknown VM {} gracefully", vm_id);
