...
use smoothtask_core::logging::async_logging::{
    write_log_batch_async, write_log_entry_async, AsyncLogRotator,
};
...
fn test_async_logging_integration() {
    ...
    runtime.block_on(async {
        ...
        assert!(
            ...
            "Metrics async logging should succeed"
        );
        ...
        let metrics_batch_result =
            log_metrics_batch_async(&metrics_batch_path, &metrics_batch).await;
        assert!(
            metrics_batch_result.is_ok(),
            "Metrics batch async logging should succeed"
        );
        ...
        let classification_result =
            log_classification_async(&classification_path, "Test classification data").await;
        assert!(
            classification_result.is_ok(),
            "Classification async logging should succeed"
        );
        ...
        let classification_batch_result =
            log_classification_batch_async(&classification_batch_path, &classification_batch).await;
        assert!(
            classification_batch_result.is_ok(),
            "Classification batch async logging should succeed"
        );
        ...
        assert!(
            policy_batch_result.is_ok(),
            "Policy batch async logging should succeed"
        );
    });
}
...
fn test_async_logging_with_rotation() {
    ...
    runtime.block_on(async {
        ...
        assert!(
            !log_path.exists(),
            "Original log file should be removed after rotation"
        );
...
fn test_async_logging_error_handling() {
    ...
    runtime.block_on(async {
        ...
        assert!(
            batch_result.is_err(),
            "Batch writing to invalid path should fail"
        );
    });
}
...
fn test_async_logging_performance() {
    ...
    runtime.block_on(async {
        ...
        println!(
            "Performance test completed in {:?} for {} entries",
            duration, batch_size
        );
        ...
        assert!(
            metadata.len() > 0,
            "Performance test log file should not be empty"
        );
    });
}
