...
fn test_performance_measurement() {
    ...
    assert!(
        system_duration.as_secs() < 5,
        "System metrics collection took too long"
    );
    assert!(
        process_duration.as_secs() < 5,
        "Process metrics collection took too long"
    );
    assert!(
        config_duration.as_secs() < 5,
...
fn test_process_data_processing() {
    ...
    assert!(
        duration.as_secs() < 2,
        "Process data processing took too long"
    );
    assert!(
        !filtered.is_empty() || processes.is_empty(),
