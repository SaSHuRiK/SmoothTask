...
fn test_batch_async_logging() {
    ...
    runtime.block_on(async {
        ...
        let entries = vec!["Batch entry 1", "Batch entry 2", "Batch entry 3"];
        ...
        let content = tokio::fs::read_to_string(&log_path)
            ...
            .expect("read file");
        assert!(
            content.contains("Batch entry 1"),
            "Log should contain first entry"
        );
        assert!(
            content.contains("Batch entry 2"),
            "Log should contain second entry"
        );
        assert!(
            content.contains("Batch entry 3"),
            "Log should contain third entry"
        );
    });
}
