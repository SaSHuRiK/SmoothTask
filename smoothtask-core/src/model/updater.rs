...
impl ModelUpdater {
    ...
    pub fn check_for_updates(
        &self,
        remote_manifest_url: impl AsRef<str>,
    ) -> Result<Vec<ModelUpdateInfo>> {
        ...
        tracing::info!("Найдено {} доступных версий на сервере", updates.len());
    ...
    pub fn download_and_install_update(&mut self, update_info: &ModelUpdateInfo) -> Result<()> {
        ...
        if !self.config.models_directory.exists() {
            fs::create_dir_all(&self.config.models_directory).with_context(|| {
                format!(
        ...
        let mut response = self
            ...
            .send()
            .with_context(|| {
                format!("Не удалось загрузить модель с {}", update_info.download_url)
            })?;
        ...
        let mut file = fs::File::create(&model_path)
            .with_context(|| format!("Не удалось создать файл модели: {}", model_path.display()))?;
    ...
    ///
    /// Результат операции.
    pub fn check_and_install_updates(
        &mut self,
        remote_manifest_url: impl AsRef<str>,
    ) -> Result<()> {
        ...
        tracing::info!("Успешно установлено {} обновлений", new_updates.len());
...
mod tests {
    ...
    fn test_model_update_info_serialization() {
        ...
        assert_eq!(
            deserialized.metadata.get("accuracy"),
            Some(&"0.95".to_string())
        );
    }
    ...
    fn test_model_updater_full_workflow() {
        ...
        assert_eq!(
            deserialized_config.models_directory,
            updater.config.models_directory
        );
        ...
        assert_eq!(
            deserialized_update.download_url,
            "https://example.com/model_v3.0.0.onnx"
        );
        assert_eq!(
            deserialized_update.metadata.get("accuracy"),
            Some(&"0.95".to_string())
        );
    }
