...
impl ModelVersion {
    ...
    pub fn compute_hash(&mut self) -> Result<&str> {
        ...
        let model_data = std::fs::read(&self.model_path).with_context(|| {
            format!(
...
impl ModelVersionManager {
    ...
    pub fn get_version(&self, version_id: impl AsRef<str>) -> Option<&ModelVersion> {
        self.versions
            .iter()
            .find(|v| v.version_id == version_id.as_ref())
    }
    ...
    pub fn rollback(&mut self) -> bool {
        ...
        let current_index = self.current_version.as_ref().and_then(|current_id| {
            self.versions
                .iter()
                .position(|v| &v.version_id == current_id)
        });
    ...
    pub fn has_version(&self, version_id: impl AsRef<str>) -> bool {
        self.versions
            .iter()
            .any(|v| v.version_id == version_id.as_ref())
    }
...
pub mod utils {
    ...
    pub fn save_versions_to_file(
        ...
        fs::write(output_path, json_data).with_context(|| {
            format!(
                "Не удалось сохранить версии в файл: {}",
                output_path.display()
            )
        })
    }
    ...
    ///
    /// Вектор ModelVersion или ошибку.
    pub fn load_versions_from_file(input_path: impl AsRef<Path>) -> Result<Vec<ModelVersion>> {
        let input_path = input_path.as_ref();
        let json_data = fs::read_to_string(input_path).with_context(|| {
            format!("Не удалось прочитать файл версий: {}", input_path.display())
        })?;

        serde_json::from_str(&json_data).context("Не удалось десериализовать версии моделей")
    }
}
...
mod tests {
    ...
    fn test_model_version_with_metadata() {
        ...
        assert_eq!(
            version.metadata.get("dataset"),
            Some(&"training_v1".to_string())
        );
    }
    ...
    fn test_utils_save_and_load_versions() {
        ...
        assert_eq!(
            loaded_versions[0].metadata.get("accuracy"),
            Some(&"0.95".to_string())
        );
    }
