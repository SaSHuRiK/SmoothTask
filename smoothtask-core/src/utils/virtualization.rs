...
fn try_read_vm_metrics_from_qemu(vm_id: &str) -> Option<SimulatedVmData> {
    ...
    debug!(
        "Attempting to read VM metrics from QEMU monitor for: {}",
        vm_id
    );
...
pub fn collect_vm_cpu_metrics(vm_id: &str) -> Result<f32, io::Error> {
    ...
    if let Some(data) = try_read_vm_metrics_from_xml(vm_id) {
        debug!(
            "Successfully read CPU metrics from libvirt XML for VM: {}",
            vm_id
        );
    ...
    if let Some(data) = try_read_vm_metrics_from_qemu(vm_id) {
        debug!(
            "Successfully read CPU metrics from QEMU monitor for VM: {}",
            vm_id
        );
        return Ok(data.cpu_usage);
    }
...
pub fn collect_vm_memory_metrics(vm_id: &str) -> Result<u64, io::Error> {
    ...
    if let Some(data) = try_read_vm_metrics_from_xml(vm_id) {
        debug!(
            "Successfully read memory metrics from libvirt XML for VM: {}",
            vm_id
        );
    ...
    if let Some(data) = try_read_vm_metrics_from_qemu(vm_id) {
        debug!(
            "Successfully read memory metrics from QEMU monitor for VM: {}",
            vm_id
        );
        return Ok(data.memory_usage);
    }
...
pub fn collect_vm_network_metrics(vm_id: &str) -> Result<VmMetrics, io::Error> {
    ...
    if let Some(data) = try_read_vm_metrics_from_xml(vm_id) {
        debug!(
            "Successfully read network metrics from libvirt XML for VM: {}",
            vm_id
        );
    ...
    if let Some(data) = try_read_vm_metrics_from_qemu(vm_id) {
        debug!(
            "Successfully read network metrics from QEMU monitor for VM: {}",
            vm_id
        );
        let mut metrics = VmMetrics::default();
        metrics.network_rx_bytes = data.network_rx_bytes;
...
pub fn collect_vm_system_metrics(vm_id: &str) -> Result<VmMetrics, io::Error> {
    ...
    if let Some(data) = try_read_vm_metrics_from_xml(vm_id) {
        debug!(
            "Successfully read system metrics from libvirt XML for VM: {}",
            vm_id
        );
        let mut metrics = VmMetrics::default();
    ...
    if let Some(data) = try_read_vm_metrics_from_qemu(vm_id) {
        debug!(
            "Successfully read system metrics from QEMU monitor for VM: {}",
            vm_id
        );
        let mut metrics = VmMetrics::default();
        metrics.uptime_seconds = data.uptime_seconds;
...
pub fn collect_vm_disk_metrics(vm_id: &str) -> Result<VmMetrics, io::Error> {
    ...
    if let Some(data) = try_read_vm_metrics_from_xml(vm_id) {
        debug!(
            "Successfully read disk metrics from libvirt XML for VM: {}",
            vm_id
        );
        let mut metrics = VmMetrics::default();
    ...
    if let Some(data) = try_read_vm_metrics_from_qemu(vm_id) {
        debug!(
            "Successfully read disk metrics from QEMU monitor for VM: {}",
            vm_id
        );
        let mut metrics = VmMetrics::default();
        metrics.disk_read_bytes = data.disk_read_bytes;
...
pub fn collect_vm_metrics(vm_id: &str) -> Result<VmMetrics, io::Error> {
    debug!(
        "Starting comprehensive metrics collection for VM: {}",
        vm_id
    );
