...
impl ProcessGrouper {
    ...
    ///
    /// Функция анализирует список процессов и группирует их в отдельные деревья процессов.
    /// Каждое дерево состоит из корневого процесса (root) и всех его потомков, которые
    /// присутствуют в переданном списке PID.
    ///
    /// # Алгоритм
    ///
    /// 1. Для каждого необработанного PID в списке:
    ///    - Находим корневой процесс (процесс, у которого родитель не в списке или является init)
    ///    - Собираем всех потомков корневого процесса, используя обход в глубину
    ///    - Добавляем группу в результат
    ///
    /// 2. Процессы, которые уже обработаны, пропускаются для предотвращения дублирования
    ///
    /// # Аргументы
    ///
    /// * `pids` - список PID процессов для группировки
    /// * `pid_to_process` - маппинг PID -> ProcessRecord для быстрого доступа к информации о процессах
    ///
    /// # Возвращает
    ///
    /// HashMap, где:
    /// - ключ - уникальный ID группы дерева (usize, последовательный номер)
    /// - значение - список PID процессов в этом дереве
    ///
    /// # Примеры
    ///
    /// ```
    /// // Пример 1: Одиночное дерево процессов
    /// // Процессы: 1000 (root, ppid=1) -> 1001, 1002 (дети)
    /// // Функция вернёт одну группу с тремя процессами [1000, 1001, 1002]
    /// ```
    ///
    /// ```
    /// // Пример 2: Множественные независимые деревья
    /// // Дерево 1: 1000 (root, ppid=1) -> 1001 (ppid=1000)
    /// // Дерево 2: 2000 (root, ppid=1) -> 2001, 2002 (ppid=2000)
    /// // Функция вернёт две группы:
    /// // - Группа 0: [1000, 1001]
    /// // - Группа 1: [2000, 2001, 2002]
    /// ```
    ///
    /// ```
    /// // Пример 3: Процессы без связей
    /// // Процессы: 1000 (ppid=1), 2000 (ppid=1), 3000 (ppid=1)
    /// // Функция вернёт три отдельные группы, каждая с одним процессом
    /// ```
    ///
    /// # Примечания
    ///
    /// - Функция обрабатывает только процессы, присутствующие в списке `pids`
    /// - Процессы с родителями вне списка считаются корневыми
    /// - Если процесс не найден в `pid_to_process`, он всё равно может быть включён в группу
    ///   (если его родитель найден), но поиск корневого процесса может завершиться на нём
