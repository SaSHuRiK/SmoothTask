...
pub trait HealthNotificationService: Send + Sync {
    ...
    async fn send_health_status_change_notification(
        &self,
...
impl HealthNotificationService for HealthNotificationServiceImpl {
    async fn send_health_notification(&self, issue: &HealthIssue) -> Result<()> {
        ...
        if let Some(last_time_value) = *last_time {
            ...
            if (duration_since_last.num_seconds() as u64)
                < settings.max_notification_frequency_seconds
            {
...
pub fn create_health_notification_service(
    settings: NotificationSettings,
) -> HealthNotificationServiceImpl {
        ...
        match event {
            HealthEvent::HealthStatusChanged {
                ...
                self.notification_service
                    .send_health_status_change_notification(old_status, new_status)
                    .await?;
            }
            ...
            HealthEvent::CriticalHealthDetected { issue, .. } => {
                self.notification_service
                    .send_critical_health_notification(&issue)
                    .await?;
            }
        }
...
pub fn create_notification_health_event_handler(
    notification_service: HealthNotificationServiceImpl,
) -> NotificationHealthEventHandler {
