...
impl ProcessEnergyMonitor {
    ...
    pub async fn collect_process_energy(&self, pid: i32) -> Result<Option<ProcessEnergyStats>> {
        ...
        match self.try_collect_proc_power_energy(pid).await {
            Ok(Some(stats)) => {
                tracing::debug!("Successfully collected process energy from /proc/power for PID {}", pid);
        ...
        if self.enable_ebpf {
            match self.try_collect_ebpf_energy(pid).await {
                Ok(Some(stats)) => {
                    tracing::debug!("Successfully collected process energy from eBPF for PID {}", pid);
                    return Ok(Some(stats));
                }
                Ok(None) => {
        ...
        if self.enable_rapl {
            match self.try_collect_rapl_energy(pid).await {
                Ok(Some(stats)) => {
                    tracing::debug!("Successfully collected process energy from RAPL for PID {}", pid);
                    return Ok(Some(stats));
                }
                Ok(None) => {
        ...
        if !errors.is_empty() {
            tracing::info!("No process energy data available for PID {} from any source. Errors: {}", 
                pid, errors.join(", "));
        } else {
            tracing::debug!("No process energy data available for PID {} - all sources returned None", pid);
        }
            ...
            if parts.len() >= 14 {
                ...
                if let (Ok(utime), Ok(stime)) = (parts[13].parse::<u64>(), parts[14].parse::<u64>()) {
                    ...
                    if system_uptime > 0 {
                        ...
                        let cpu_usage = (total_time as f32 / system_uptime as f32 * 100.0).min(100.0);
    ...
    async fn get_system_uptime(&self) -> Result<u64> {
        if let Ok(uptime_content) = tokio_fs::read_to_string("/proc/uptime").await {
            if let Some(first_line) = uptime_content.lines().next() {
                if let Some(uptime_secs) = first_line.split_whitespace().next().and_then(|s| s.parse::<f64>().ok()) {
    ...
    async fn get_cached_ebpf_metrics(&self) -> Result<Option<crate::metrics::ebpf::EbpfMetrics>> {
        ...
        if ebpf_metrics.is_none() {
            tracing::debug!("eBPF metrics not available - eBPF support may be disabled or not initialized");
        } else {
    ...
    async fn try_collect_rapl_energy(&self, pid: i32) -> Result<Option<ProcessEnergyStats>> {
        ...
        let process_energy_uj = self.calculate_process_energy_from_rapl(
            &domain_energy,
    ...
    fn extract_cpu_id_from_domain(&self, domain_path: &std::path::Path) -> Option<usize> {
        if let Some(file_name) = domain_path.file_name() {
            if let Some(name_str) = file_name.to_str() {
                ...
                if name_str.contains("package") || name_str.contains("core") || name_str.contains("uncore") {
...
mod tests {
    ...
    async fn test_sync_wrapper() {
        ...
        let result = std::thread::spawn(move || {
            monitor.collect_process_energy_sync(999999)
        }).join().unwrap();
    ...
    async fn test_energy_calculation() {
        ...
        let process_energy = monitor.calculate_process_energy_from_rapl(&domain_energy, &cpu_affinity);
