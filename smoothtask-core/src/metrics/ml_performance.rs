...
pub fn ml_metrics_to_prometheus(metrics: &MLPerformanceMetrics, model_name: &str) -> String {
...
pub fn ml_metrics_map_to_prometheus(metrics_map: &HashMap<String, MLPerformanceMetrics>) -> String {
    let mut output = String::new();
...
impl MLPerformanceReport {
    ...
    pub fn from_metrics(metrics: &MLPerformanceMetrics, model_name: impl Into<String>) -> Self {
        Self {
...
pub fn create_ml_performance_reports(
    ...
    metrics_map
        .iter()
        .map(|(model_name, metrics)| MLPerformanceReport::from_metrics(metrics, model_name))
        .collect()
}
...
mod tests {
    ...
    fn test_empty_metrics() {
        ...
        assert!(
            prometheus_output.contains("ml_classifications_successful{model=\"empty_model\"} 0")
        );
