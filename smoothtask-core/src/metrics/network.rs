...
impl NetworkMonitor {
    ...
    /// Get interface flags with improved error handling
    fn get_interface_flags(&self, name: &str) -> Result<u32> {
        let flags_path = format!("/sys/class/net/{}/flags", name);
        let flags_str = fs::read_to_string(flags_path)
            .with_context(|| format!("Failed to read flags for interface {}", name))?;
        let flags = flags_str.trim().parse::<u32>()
    ...
    /// Get MAC address with error handling
    ...
    fn get_ip_addresses(&self, name: &str) -> Vec<IpAddr> {
        ...
        if let Ok(ipv4_dir) = fs::read_dir(ipv4_path) {
            ...
            tracing::debug!("No IPv4 addresses found for interface {}", name);
        }
        ...
        if let Ok(ipv6_dir) = fs::read_dir(ipv6_path) {
            ...
            tracing::debug!("No IPv6 addresses found for interface {}", name);
        }

        addresses
    }

    /// Check if interface is up with error handling
