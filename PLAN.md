...
- [ ] ST-306: Тесты для build_feature_matrix без части колонок
  - Тип: Python / trainer / tests
  - Критерии готовности:
    - Смоделированы входные данные без необязательных полей;
    - Добавлен unit-тест в `smoothtask-trainer/tests/test_features.py`;
    - Тест подтверждает корректное заполнение default-значений.

- [ ] ST-307: Мини-код-ревизия API ответов на пустые паттерны
  - Тип: Rust / core / api / tests
  - Критерии готовности:
    - Проверена обработка `/api/patterns` при пустом списке паттернов;
    - Добавлен тест, фиксирующий ожидаемую структуру ответа;
