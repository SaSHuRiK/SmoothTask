...
- [ ] ST-045: Реализация probe-thread для измерения scheduling latency (mini-cyclictest)
  ...
  - Критерии готовности:
    ...
    - Unit-тесты на измерение latency и вычисление перцентилей.
  - Примечания: согласно документации tz.md, scheduling latency измеряется через mini-cyclictest поток, который спит на 5-10 мс и меряет dt = wakeup_delay. Это более сложная задача, требующая создания отдельного потока и управления его жизненным циклом.
