...
- [~] ST-028: Базовые интеграционные тесты для главного цикла демона (run_daemon)
  ...
  - Критерии готовности:
    ...
    - [~] Все тесты проходят без реальных системных вызовов (тесты компилируются, но зависают из-за бесконечного цикла демона; требуется доработка механизма прерывания).
  - Примечания: создан файл tests/run_daemon_test.rs с 5 интеграционными тестами. Тесты компилируются и один тест (test_daemon_handles_nonexistent_patterns_dir) проходит успешно. Остальные тесты зависают из-за того, что timeout не прерывает бесконечный цикл демона должным образом. Требуется доработка: либо добавить механизм graceful shutdown в демон, либо использовать другой подход к тестированию (например, проверять только инициализацию без запуска цикла).
