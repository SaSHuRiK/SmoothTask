...
- [x] ST-044: Добавить комментарий TODO для scheduling latency в collect_snapshot
  - Тип: Rust / core / документация
  - Критерии готовности:
    - Добавлен комментарий TODO в collect_snapshot, объясняющий, что scheduling latency пока не собирается;
    - Все тесты проходят (155 unit-тестов).
  - Примечания: добавлен комментарий TODO в функцию collect_snapshot, объясняющий, что поля sched_latency_p95_ms и sched_latency_p99_ms пока остаются None и будут реализованы в задаче ST-045. Все 155 тестов проекта проходят успешно.
...
- [x] ST-046: Создать структуру LatencyCollector для хранения измерений scheduling latency и вычисления перцентилей
  ...
  - Критерии готовности:
    - Создан модуль `metrics::scheduling_latency` с структурой `LatencyCollector`;
    - Поддержка скользящего окна измерений (max_samples);
    - Методы для добавления измерений (`add_sample`) и вычисления перцентилей (`percentile`, `p95`, `p99`);
    - Потокобезопасность для использования из probe-thread;
    - Unit-тесты на все компоненты (12 тестов).
...
- [ ] ST-045: Реализация probe-thread для измерения scheduling latency (mini-cyclictest)
  - Тип: Rust / core / metrics
  - Критерии готовности:
    - Создан отдельный поток (probe-thread), который периодически спит и измеряет задержки пробуждения;
    - Использование LatencyCollector из ST-046 для хранения измерений;
    - Сбор статистики (P95, P99) за окно времени;
    - Интеграция в collect_snapshot для заполнения sched_latency_p95_ms и sched_latency_p99_ms;
    - Unit-тесты на измерение latency и интеграцию с collect_snapshot.
  - Примечания: согласно документации tz.md, scheduling latency измеряется через mini-cyclictest поток, который спит на 5-10 мс и меряет dt = wakeup_delay. Это более сложная задача, требующая создания отдельного потока и управления его жизненным циклом. Зависит от ST-046 (LatencyCollector).
...
- [ ] ST-011: Реализация WaylandIntrospector для поддержки Wayland-композиторов
  ...
  - Примечания: Wayland требует работы с wayland-client и специфичными протоколами композиторов. Это более сложная задача, чем X11. Базовая структура и функция проверки доступности уже реализованы в ST-039.
