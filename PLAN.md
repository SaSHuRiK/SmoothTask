...
- [ ] ST-005-2: Реальный PipeWire/PulseAudio адаптер для AudioIntrospector
  ...
  - Примечания: разбито на подзадачи:
    - [x] ST-005-2a: Парсер `pw-dump` → AudioClientInfo (выполнено)
    - [x] ST-005-2b: PipeWireIntrospector через `pw-dump` (выполнено: реализована структура и методы AudioIntrospector, отслеживание XRUN через ERR, 4 unit-теста)
    - [x] ST-005-2c: Интеграция PipeWireIntrospector в главный цикл демона (замена StaticAudioIntrospector)
    - [x] ST-005-2d: Улучшение отслеживания XRUN (выполнено: расширена поддержка форматов ERR, улучшена логика отслеживания, добавлены тесты)
  - Тип: Rust / core / metrics
  - Критерии готовности:
