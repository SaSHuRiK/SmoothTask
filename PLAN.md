...
- [x] ST-017: Построение фич для ML-ранкера в Rust (`model::features`)
  - Тип: Rust / core / model
  - Критерии готовности:
    - Функция `build_features()` для преобразования Snapshot в вектор фич для каждого AppGroup;
    - Поддержка тех же фич, что и в Python-версии (числовые, булевые, категориальные);
    - Нормализация и дефолты для отсутствующих значений (аналогично Python);
    - Возврат структуры с фичами и cat_feature_indices;
    - Unit-тесты на построение фич из тестового Snapshot (минимум 3 теста).
