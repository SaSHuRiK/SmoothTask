...
- [ ] ST-195: Добавить helper-функцию для вычисления корреляций между параметрами политики и метриками отзывчивости
  - Тип: Python / trainer / analysis
  - Критерии готовности:
    - [ ] Реализована функция compute_policy_correlations(snapshots_df);
    - [ ] Функция вычисляет корреляции между PSI-метриками и bad_responsiveness;
    - [ ] Функция вычисляет корреляции между latency-метриками и порогами;
    - [ ] Добавлены тесты для функции;
    - [ ] Все существующие тесты проходят успешно.
  - Примечания: это вспомогательная функция для будущей реализации tune_policy, которая будет использоваться для анализа корреляций.
