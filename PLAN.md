...
- [~] ST-004: Каркас WindowIntrospector и выбор фокусного окна
  ...
  - Обновления: `pid_confidence` теперь обнуляется, если PID отсутствует, добавлен юнит-тест на этот случай. Добавлены функции `get_window_info_by_pid()` и `build_pid_to_window_map()` для получения информации об окнах по PID, покрыты unit-тестами (4 новых теста, всего 12 тестов в модуле).
