...
- [ ] ST-025: Реализация X11Introspector для получения информации об окнах через EWMH (X11)
  - Тип: Rust / core / metrics
  - Критерии готовности:
    - Структура `X11Introspector`, реализующая трейт `WindowIntrospector`;
    - Получение списка окон через EWMH (`_NET_CLIENT_LIST`);
    - Получение информации об окне: title, app_id, workspace, state (focused/minimized/fullscreen);
    - Получение PID через `_NET_WM_PID`;
    - Определение фокусного окна через `_NET_ACTIVE_WINDOW`;
    - Unit-тесты на парсинг окон и обработку ошибок (минимум 3 теста);
    - Fallback на StaticWindowIntrospector, если X11 недоступен.

- [ ] ST-026: Интеграция реального evdev для InputActivityTracker (чтение событий из /dev/input)
  - Тип: Rust / core / metrics
  - Критерии готовности:
    - Структура `EvdevInputTracker`, которая читает события из `/dev/input/*`;
    - Автоматическое обнаружение доступных устройств ввода (клавиатура, мышь);
    - Подписка на события через evdev-библиотеку;
    - Интеграция с `InputActivityTracker` для обновления метрик;
    - Unit-тесты на чтение событий и обработку ошибок (минимум 2 теста);
    - Fallback на статический трекер, если evdev недоступен.
