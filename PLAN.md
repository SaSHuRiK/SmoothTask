...
- [ ] ST-197: Реализовать оптимизацию порогов latency в tune_policy (на основе перцентилей реальных значений)
  - Тип: Python / trainer / optimization
  - Критерии готовности:
    - [ ] Реализована функция optimize_latency_thresholds(snapshots_df, percentile=0.95, multiplier=1.5);
    - [ ] Функция вычисляет перцентили реальных значений latency в хороших условиях (bad_responsiveness=false);
    - [ ] Функция возвращает оптимальные значения sched_latency_p99_threshold_ms и ui_loop_p95_threshold_ms;
    - [ ] Добавлены тесты для функции;
    - [ ] Все существующие тесты проходят успешно.

- [ ] ST-198: Реализовать сохранение оптимизированного конфига в YAML
  - Тип: Python / trainer / config
  - Критерии готовности:
    - [ ] Реализована функция save_optimized_config(config_dict, config_out);
    - [ ] Функция сохраняет оптимизированные параметры в YAML файл;
    - [ ] Функция сохраняет остальные параметры из исходного конфига (если он существует);
    - [ ] Добавлены тесты для функции;
    - [ ] Все существующие тесты проходят успешно.
