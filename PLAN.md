...
- [ ] ST-060: Добавление функции проверки доступности необходимых системных утилит (pw-dump, evdev) при старте демона
  - Тип: Rust / core / daemon
  - Критерии готовности:
    - Функция проверяет доступность pw-dump и evdev устройств;
    - Логирование предупреждений, если утилиты недоступны;
    - Unit-тесты для проверки доступности (2 теста).

- [ ] ST-061: Улучшение обработки ошибок в collect_snapshot: более информативные сообщения с контекстом
  - Тип: Rust / core / daemon
  - Критерии готовности:
    - Сообщения об ошибках включают контекст (какой компонент не смог собрать метрики);
    - Улучшена обработка частичных ошибок (если один компонент не работает, остальные продолжают);
    - Unit-тесты для проверки сообщений об ошибках (2 теста).
