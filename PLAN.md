...
- [x] ST-601: Оптимизировать использование памяти в eBPF структурах
  ...
  - Изменённые файлы:
    - smoothtask-core/src/ebpf_programs/cpu_metrics_memory_optimized.c: Новая eBPF программа с оптимизацией памяти для CPU метрик, использующая 32-битные поля и битовую упаковку
    - smoothtask-core/src/ebpf_programs/gpu_monitor_memory_optimized.c: Новая eBPF программа с оптимизацией памяти для GPU мониторинга, использующая минимальные типы данных (8/16 бит)
    - smoothtask-core/src/metrics/ebpf.rs: Добавлены функции optimize_detailed_stats, set_max_cached_details, get_max_cached_details, optimize_memory_usage, optimize_ebpf_map_memory, configure_memory_optimized_mode
  - Новые функции:
    - optimize_detailed_stats: Оптимизация детализированных статистик путем ограничения количества записей
    - set_max_cached_details: Установка максимального количества кэшируемых детализированных статистик
    - get_max_cached_details: Получение текущего максимального количества кэшируемых детализированных статистик
    - optimize_memory_usage: Комплексная оптимизация использования памяти в eBPF структурах
    - optimize_ebpf_map_memory: Оптимизация памяти в eBPF картах
    - configure_memory_optimized_mode: Настройка режима оптимизации памяти
  - Результаты: Полная реализация оптимизации памяти в eBPF структурах с поддержкой ограничения детализированных статистик, очистки кэша при высоком использовании памяти, уменьшения размера batches и настройки режима оптимизации памяти

- [ ] ST-602: Улучшить документацию API для основных модулей
  ...
  - Критерии готовности:
    - ✅ Документация для metrics модуля
    - ✅ Документация для policy модуля
    - ✅ Документация для classify модуля
