...
- [x] ST-014: Дефолты кат-фич и тегов в build_feature_matrix
  ...
  - Критерии готовности:
    - Отсутствующие теги нормализуются в `unknown` вместо пустой строки;
    - Категориальные фичи получают консистентные дефолты `unknown`;
    - Unit-тесты покрывают fallback для tags/app_name/булевых/числовых фич.

- [x] ST-015: Доп. проверки build_feature_matrix (trainer)
  - Тип: Python / trainer / tests
  - Критерии готовности:
    - Ошибка на пустом DataFrame без снапшотов;
    - Проверка порядка cat_feature_indices соответствует колонкам;
    - Unit-тесты в `tests/test_features.py` проходят.

- [x] ST-016: Реализация главного цикла демона (run_daemon)
  - Тип: Rust / core / daemon
  - Критерии готовности:
    - Инициализация всех подсистем (SnapshotLogger, PolicyEngine, HysteresisTracker, интроспекторы);
    - Функция collect_snapshot для сбора всех метрик;
    - Главный цикл с последовательностью: сбор метрик → группировка → классификация → политика → actuator → логирование;
