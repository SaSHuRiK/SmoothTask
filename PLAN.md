...
- [x] ST-193: Добавить базовую валидацию входных данных в tune_policy (проверка существования БД, минимальное количество снапшотов)
  ...
  - Критерии готовности:
    - [x] Добавлена проверка существования db_path;
    - [x] Добавлена проверка минимального количества снапшотов (100);
    - [x] Добавлены соответствующие тесты;
    - [x] Все существующие тесты проходят успешно (15 тестов).
  - Примечания: добавлена валидация входных данных в tune_policy: проверка существования БД (_validate_db_path), проверка схемы БД (_validate_db_schema), проверка минимального количества снапшотов (_count_snapshots). Все 15 тестов проходят успешно.

- [x] ST-194: Добавить helper-функцию для загрузки снапшотов из БД в tune_policy
  - Тип: Python / trainer / data loading
  - Критерии готовности:
    - [x] Реализована функция load_snapshots_for_tuning(db_path, min_snapshots, days_back);
    - [x] Функция фильтрует снапшоты по временному диапазону;
    - [x] Функция проверяет минимальное количество снапшотов;
    - [x] Добавлены тесты для функции;
    - [x] Все существующие тесты проходят успешно (15 тестов).
  - Примечания: реализована функция load_snapshots_for_tuning для загрузки снапшотов с фильтрацией по времени и проверкой минимального количества данных. Функция использует валидацию БД и возвращает DataFrame со снапшотами. Все 15 тестов проходят успешно.
...
- [x] ST-195: Добавить helper-функцию для вычисления корреляций между параметрами политики и метриками отзывчивости
  ...
  - Критерии готовности:
    - [x] Реализована функция compute_policy_correlations(snapshots_df);
    - [x] Функция вычисляет корреляции между PSI-метриками и bad_responsiveness;
    - [x] Функция вычисляет корреляции между latency-метриками и bad_responsiveness/responsiveness_score;
    - [x] Добавлены тесты для функции (6 тестов);
    - [x] Все существующие тесты проходят успешно (21 тест).
  - Примечания: реализована функция compute_policy_correlations для вычисления корреляций Пирсона между PSI-метриками (psi_cpu_some_avg10, psi_io_some_avg10), latency-метриками (sched_latency_p99_ms, ui_loop_p95_ms) и метриками отзывчивости (bad_responsiveness, responsiveness_score). Функция корректно обрабатывает пустые DataFrame, отсутствующие колонки, NULL значения и недостаточное количество данных. Добавлены 6 тестов, покрывающих базовые сценарии, пустые данные, отсутствующие колонки, NULL значения, недостаточное количество данных и реальные данные из БД. Все 21 тест в test_tune_policy.py проходят успешно.
...
- [x] ST-196: Реализовать оптимизацию порогов PSI в tune_policy (на основе перцентилей PSI в моменты bad_responsiveness)
  ...
  - Критерии готовности:
    - [x] Реализована функция optimize_psi_thresholds(snapshots_df, percentile=0.95);
    - [x] Функция вычисляет перцентили PSI значений в моменты bad_responsiveness=true;
    - [x] Функция возвращает оптимальные значения psi_cpu_some_high и psi_io_some_high;
    - [x] Добавлены тесты для функции (7 тестов);
    - [x] Все существующие тесты проходят успешно (28 тестов).
  - Примечания: реализована функция optimize_psi_thresholds для оптимизации порогов PSI на основе перцентилей PSI значений в моменты bad_responsiveness. Функция корректно обрабатывает пустые DataFrame, отсутствие моментов bad_responsiveness, отсутствующие колонки, NULL значения и различные перцентили. Добавлены 7 тестов, покрывающих базовые сценарии, пустые данные, отсутствие плохих условий, отсутствующие колонки, NULL значения, различные перцентили и реальные данные из БД. Все 28 тестов в test_tune_policy.py проходят успешно.

- [x] ST-197: Реализовать оптимизацию порогов latency в tune_policy (на основе перцентилей реальных значений)
  - Тип: Python / trainer / optimization
  - Критерии готовности:
    - [x] Реализована функция optimize_latency_thresholds(snapshots_df, percentile=0.95, multiplier=1.5);
    - [x] Функция вычисляет перцентили реальных значений latency в хороших условиях (bad_responsiveness=false);
    - [x] Функция возвращает оптимальные значения sched_latency_p99_threshold_ms и ui_loop_p95_threshold_ms;
    - [x] Добавлены тесты для функции (9 тестов);
    - [x] Все существующие тесты проходят успешно (40 тестов).
  - Примечания: реализована функция optimize_latency_thresholds для оптимизации порогов latency на основе перцентилей реальных значений в хороших условиях. Функция корректно обрабатывает пустые DataFrame, отсутствие моментов с хорошими условиями, отсутствующие колонки, NULL значения, различные перцентили и множители. Добавлена логическая валидация (P99 >= P95). Добавлены 9 тестов, покрывающих базовые сценарии, пустые данные, отсутствие хороших условий, отсутствующие колонки, NULL значения, различные перцентили, различные множители и реальные данные из БД. Все 40 тестов в test_tune_policy.py проходят успешно.

- [x] ST-198: Реализовать сохранение оптимизированного конфига в YAML
  - Тип: Python / trainer / config
  - Критерии готовности:
    - [x] Реализована функция save_optimized_config(config_dict, config_out, config_in=None);
    - [x] Функция сохраняет оптимизированные параметры в YAML файл;
    - [x] Функция сохраняет остальные параметры из исходного конфига (если он существует);
    - [x] Добавлены тесты для функции (3 теста);
    - [x] Все существующие тесты проходят успешно (40 тестов).
  - Примечания: реализована функция save_optimized_config для сохранения оптимизированного конфига в YAML файл. Функция поддерживает рекурсивное обновление вложенных словарей, сохранение остальных параметров из исходного конфига (если он указан) и корректную обработку ошибок (FileNotFoundError, IOError, PermissionError). Функция tune_policy обновлена для использования новых функций optimize_latency_thresholds и save_optimized_config. Добавлены 3 теста, покрывающих базовое сохранение, сохранение с базовым конфигом и обработку несуществующего базового конфига. Все 40 тестов в test_tune_policy.py проходят успешно.
