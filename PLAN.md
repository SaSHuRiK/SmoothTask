...
- [ ] ST-614: Добавить поддержку мониторинга температуры CPU через eBPF
  ...
  - Критерии готовности:
    - Улучшенная eBPF программа для мониторинга температуры CPU
    - Новый API endpoint для получения информации о температуре
    - Интеграция с существующим eBPF модулем
    - Комплексные тесты для новой функциональности

- [ ] ST-615: Улучшить обработку ошибок в модуле метрик процессов
  ...
  - Критерии готовности:
    - Улучшенные сообщения об ошибках в process.rs
    - Добавлены практические рекомендации по устранению неполадок
    - Улучшена обработка граничных случаев
    - Комплексные тесты для новых сценариев обработки ошибок

- [ ] ST-616: Оптимизировать использование памяти в eBPF картах
...
- [x] ST-613: Обновить документацию для новых функций и изменений
  ...
  - Результаты: Проверка показала, что документация уже обновлена и актуальна. API.md содержит полную документацию для новых endpoint'ов (/api/cache/stats, /api/cache/clear, /api/cache/config, /api/network/connections) с примерами использования и конфигурации. EBPF документация также обновлена с информацией о новых функциях мониторинга сетевых соединений и фильтрации.
